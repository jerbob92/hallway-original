...
function ijodRequest(endpoint, basePath, range, cbEach, cbDone) {
  ...
  profileManager.loadProfile(pid, function(err, profile) {
    ...
    if (!profile.pod && range.app && lconfig.pods.apps && lconfig.pods.apps[range.app])
    {
      ...
      if(profile.pod == 1 && profile.cat && Date.parse(profile.cat) < 1367712000000) profile.pod = false;
...
exports.getOne = function(targetIdr, profileHints, cbDoneOrig, app) {
  ...
  function getPodOne(pid) {
    profileManager.loadProfile(pid, function(err, profile) {
      if (err) return cbDone(err);
      if (!profile || !profile.pod || !lconfig.pods.enabled) return nexusClient.getOne(targetIdr, cbDone);
  ...
  async.forEach(Object.keys(bases), function(base, cbStep) {
    ...
    if (pid) {
      ...
      profileManager.loadProfile(pid, function(err, profile) {
        ...
        if (!err && !profile) logger.warn('Profile missing for', base, pid);
        if (err || !profile || !profile.pod) {

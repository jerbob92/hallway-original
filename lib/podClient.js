...
exports.getOne = function(targetIdr, profileHints, cbDone) {
  ...
  function getPodOne(pid) {
    profileManager.loadProfile(pid, function(err, profile) {
      if (err) return cbDone(err);
      if (!profile) return nexusClient.getOne(targetIdr, cbDone);
...
exports.piplineInject = function(bases, auth, cbDone) {
  ...
  Object.keys(bases).forEach(function(base) {
    ...
    if (pid) {
      ...
      profileManager.loadProfile(pid, function(err, profile) {
        if (err && !profile.pod) {
          return nexusClient.pipelineInject(base, bases[base], auth, cbDone);
        }
        // Send it to the pod

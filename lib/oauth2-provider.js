...
OAuth2Provider.prototype.generateAccessToken = function(user_id, client_id, extra_data, template) {
  var out = template || {};
  out.access_token = this.serializer.stringify([user_id, client_id, +new Date(), extra_data]);
...
OAuth2Provider.prototype.oauth = function() {
  ...
  return connect.router(function(app) {
    ...
    var auth_get_fn = function(req, res, next) {
    ...
    app.get('/oauth/authorize', auth_get_fn);
    app.get('/oauth/authenticate', auth_get_fn);

    // Function for processing POST requests to /oauth/[authenticate|authorize]
    var auth_post_fn = function(req, res, next) {
      var     client_id = req.query.client_id,
           redirect_uri = req.query.redirect_uri,
      ...
      if('allow' in req.body) {
        if('token' === response_type) {
          ...
          self.emit('create_access_token', user_id, client_id, function(extra_data, template) {
            url += querystring.stringify(self.generateAccessToken(user_id, client_id, extra_data, template));
    ...
    app.post('/oauth/authorize', auth_post_fn);
    app.post('/oauth/authenticate', auth_post_fn);
    ...
    app.post('/oauth/access_token', function(req, res, next) {
      ...
      self.emit('lookup_grant', client_id, client_secret, code, function(err, user_id) {
        ...
        self.emit('create_access_token', user_id, client_id, function(extra_data, template) {
          res.end(JSON.stringify(self.generateAccessToken(user_id, client_id, extra_data, template)));
        });

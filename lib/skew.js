...
Skew.prototype.setFilled = function(callback) {
  this.redisClient.set('filled', 'true', function(err, reply) {
    callback(err);
  });
};
...
Skew.prototype.getFillLock = function(callback) {
  ...
  self.redisClient.setnx('lock:fill', Date.now() + (30 * 60 * 1000), function(err, setNxReply) {
    if (err) {
      return callback(err, false);
    }

    console.error('XXX setNxReply', util.inspect(setNxReply));

    // The lock is held by someone else, check if it's expired
    if (setNxReply === 0) {
      self.redisClient.get('lock:fill', function(err, getReply) {
        console.error('XXX getReply', util.inspect(getReply));

        // Is it expired?
        if (Date.now() > parseInt(getReply, 10)) {
          // Set the expiration and get the old value (to make sure another worker didn't beat us)
          self.redisClient.getset('lock:fill', Date.now() + (30 * 60 * 1000), function(err, getSetReply) {
            console.error('XXX getSetReply', util.inspect(getSetReply));

            // If the old value is still expired then we now hold the lock
            callback(err, Date.now() > parseInt(getSetReply, 10));
          });
        } else {
          callback(err, false);
        }

...
//    var account = getACookie(req, app.app);
    ...
    acl.getProfiles(account, function(err, profiles) {
      ...
      profiles.forEach(function(item){
        ...
        if (item.profile.indexOf(req.query.service) > 0) found = item.profile;
      });
      ...
      if (found) {
        ...
        return cbForm({id:account, pid:found});
      }
...
myOAP.on('save_grant', function(user, client_id, code, callback) {
  logger.debug("grant saving",code, user, client_id);
  acl.addGrant(code, user.id, client_id, user.pid, function(err){
...
myOAP.on('create_access_token', function(user, client_id, callback) {

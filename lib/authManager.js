...
exports.batchApplyAuth = function(req, res, appID, appSecret, accounts, callback) {
  acl.getApp(appID, function(err, app) {
    ...
    async.forEachSeries(accounts, function(account, cbAccount) {
      var id = account.id;
      delete account.id;
      ...
      async.forEachSeries(services, function(service, cbService) {
        ...
        finishAuth(req, res, service, auth, appID, function(err, newAccount) {
          if (err) {
            if (!thisAccount.err) thisAccount.error = {};
            thisAccount.error[service] = err;
            return cbService();
          }
...
function finishAuth(req, res, service, auth, appID, callback) {
  ...
  self.sync({auth:auth, config:{}}, function(err, data){
    ...
    var existing = getACookie(req, appID).id;
    if(auth.account) existing = auth.account;
    if(existing == "false") existing = false; // allow account=false passed in to authorize to force it to be a login/signup regardless
    acl.getOrAdd(existing, appID, auth.pid, function(err, account, count) {

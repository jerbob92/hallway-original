...
var myOAP = new OAuth2Provider(lconfig.authSecrets.crypt, lconfig.authSecrets.sign, lconfig.authSecrets.oldcrypt, lconfig.authSecrets.oldsign);
...
function startServiceAuth(service, appID, req, res) {
  ...
  if(fs.existsSync(path.join(__dirname, 'services', service)))
  {
    ...
    acl.getApp(service, function(err, app){
      if(err || !app || !app.notes.apiauth) return authfail(req, service, res, "Unknown service: "+sanitizer.escape(service), appID);
...
function finishAuth(req, res, service, auth, appID, callback) {
  ...
  self.sync({auth:auth, config:{}}, function(err, data){
    ...
    // Parse out original entry URL so we can check for either authenticate
    // (NEW) or authorize (OLD) behaviour (note we pass "true" to url.parse so
    // that we get query parameters parsed out)
    var origReqUrl = urllib.parse(req.cookies.callback, true);
    var accountID = false;
    if (origReqUrl.pathname == "/oauth/authorize") {
      // OLD behaviour -- look for existing account for this app and use that to link this new
      // account with it
      accountID = getACookie(req, appID).id;
      if (auth.account) accountID = auth.account;
      if (accountID == "false") accountID = false;
    } else {
      // NEW behaviour -- use access_key, if present, to specify account ID
    ...
    acl.getOrAdd(accountID, appID, auth.pid, function(err, account, count) {
      ...
      auth.accounts[account.account] = Date.now(); // track every account associated to this profile and app, ALSO see authMerge below does this
      async.series([
        function(cb) {
        ...
        function() {
          ...
          taskman.taskUpdate(auth, function() {}); // save auth, set up synclet tasks, and forces immediate sync too
          if(res.applied_flag) {
            account.access_token = myOAP.serializer.stringify([account.account, appID, +new Date(), null]);
...
var FirebaseAuth = require("./firebase-auth-server.js");
...
myOAP.on('create_access_token', function(account_id, client_id, callback) {
  ...
  apiKeys.getKeys("firebase", client_id, function(keys){
    ...
    var tokenGenerator = new FirebaseAuth(keys.appKey, keys.appSecret);
    var firebaseToken = tokenGenerator.createAuthToken({account: account_id});
    callback(null, {firebase:firebaseToken});
  });
});

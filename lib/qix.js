...
exports.buf = function(str, sensitive, buf) {
  buf = buf || new Buffer(new Array(32));
  if (!str) return undefined;
  var tokens = tok(str);
  var arr;
  // if sensitively indexing, use just first 8 chars of raw string only of any
  // size
  if (sensitive) {
    arr = tokens.map(function(str) {
      return str.toLowerCase().substr(0,8);
    });
  } else {
    arr = defl8(tokens);
  }
  ...
  arr.forEach(function(str) {
    shifty(str, buf, sensitive);
  });
  return buf;
};

exports.chunk = function(str, sensitive) {
  if (sensitive) return tok(str).map(function(s){ return s.toLowerCase().substr(0,8); });
  return defl8(tok(str));
};

// micro index a string into a fixed buffer by shifting bits
function shifty(str, buf, sensitive) {
  var mask = 0x1;
  var sum = 0;
  var code;
  for(var i=0; i < str.length; i++) {
    code = str.charCodeAt(i);
    // alpha preserved, rest punctuated, to get our micro index
    code = (code > 96 && code < 123) ? code - 97 : 26;
    buf[code] |= mask; // flip the bit at this offset for this character
    mask <<= 1; // move to the next bit
    sum += code;
  }
  // if a smaller word, throw in terminator
  if (!sensitive && str.length !== 8) buf[27+(code % 4)] |= mask;
  return buf;
}
...
function defl8(arr) {
  ...
  for(var i=0;i < arr.length; i++) {
    ...
    if (str.length <= 8 && str.split(/\D+/).length > 1) {
      ret.push(str);
      continue;
    } // smaller w/ some alpha in them
    // rest hash'd
    var x = mmh.murmur32HexSync(str);
    var ss = "";
    // use only the first 8 nibbles of the hash and make as unique a key as
    // possible translating into the unpopular character ranges
    for(var j=0; j < 8; j++)
    {
...
function tok(str) {
  ...
  for(var i=0;i<str.length;i++)
  {
    ...
    // we're allowing alphanumerics and all unicode/higher chars as string
    // sequences
    if (
      (code > 47 && code < 58) || // 0-9
      (code > 64 && code < 91) || // A-Z

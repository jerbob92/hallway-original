...
{
  if(!lconfig.partition || !lconfig.partition.size) return 'Entries';
  return 'Entries_'+partition.substr(0, lconfig.partition.size).toLowerCase();
}
...
exports.readFrom = function(id, cbDone)
{
  ...
  if(idp.part) parts.unshift({dal:partdb, hash:idp.hash, table:idp.table, partition:idp.part, trump:true});
  cbDone(parts);
};

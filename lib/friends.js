...
// what the custom saved address book namespaces are
var MOBILE_GROUPS = {
  "android":{"base":"data:id@service/friends_android", "path":"friends_android"},
  "ios":{"base":"data:id@service/friends_ios", "path":"friends_ios"}
}

// our format for saving address book entries needs to have a map to be normalized like anything else
var MOBILE_MAP = {
  oembed: function(data, base) {
    var ret = {type:'contact'};
    // allow some plural versions
    if(Array.isArray(data.urls) && data.urls.length > 0) ret.url = data.urls[0];
    if(Array.isArray(data.emails) && data.emails.length > 0) ret.url = data.emails[0];
    if(Array.isArray(data.phones) && data.phones.length > 0) ret.phone = data.phones[0];
    ret.title = data.name;
    ...
    ret.provider_name = base.path.split('_')[1]; // the friends_type pathing
    ret.id = data.id;
    return ret;
  },
  'type':'contacts'
}
Object.keys(MOBILE_GROUPS).forEach(function(group){ dMap.loadcustom('data', MOBILE_GROUPS[group].path, MOBILE_MAP); });

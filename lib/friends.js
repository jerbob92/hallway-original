...
function interactive(auth, iact, cbDone)
{
  ...
  async.forEach(bases, function(base, cbBase){
    ...
    if(options.in.length == 0) return cbBase();
    ...
    ijod.getPars(base, options, function(err, pars){
      ...
      async.forEachLimit(Object.keys(pars), 10, function(idh, cbPars){
        if(pars[idh].length == 0) return async.nextTick(cbPars); // skip non-indexed entries
        ...
        ijod.setOnePars(idr.toString(id), "outer", outers, function(){async.nextTick(cbPars)});
      }, function(){
        async.nextTick(cbBase);
      });
    });
  }, cbDone);
}

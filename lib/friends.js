...
// change this to force re-indexing of any contact info on next run, whenever the indexing logic here changes
var VERSION = 6;
...
exports.bump = function(cset, auth, cbDone) {
  ...
  cset.forEach(function(entry){
    ...
    // very special case, this one is our own contact entry from a device
    var id = idr.parse(entry.idr);
    if(id.host == 'devices' && id.protocol == 'contact' && id.path == 'self') dself = entry;
    ...
    { // below we do a lot more work on contacts
      var id = idr.parse(entry.idr);
      if(id.host == 'devices') deviceId = id.auth.split('.'); // a device contact id is in the format devicename.accountid.appid@devices
  ...
  async.waterfall([
    function(cb){ (dself) ? deviceMe(dself, cb) : cb(); },
    function(cb){ friendex(auth, pids, cb); },
    function(cb){ (deviceId) ? devicePeers(deviceId[1], deviceId[2], pids, cb) : peerCheck(auth, pids, cb); }, // device contacts are special, per-app
...
function deviceIds(entry)
{
  var ids = {};
  if(entry.data.phone) ids[phoneHome(entry.data.phone)] = true;
  if(entry.data.email) ids[entry.data.email.toLowerCase()] = true;
  if(Array.isArray(entry.data.phones)) entry.data.phones.forEach(function(phone){ ids[phoneHome(phone)] = true });
  if(Array.isArray(entry.data.emails)) entry.data.emails.forEach(function(email){ ids[email.toLowerCase()] = true });
  return ids;
}

// we need to index our own unique identifiers in an easily matchable way to find peers in an app
function deviceMe(entry, cbDone)
{
  // first get all the unique ids
  var ids = deviceIds(entry);

  // now for each one save this account to it's app-wide entry
  var parts = idr.parse(entry.idr).auth.split('.');
  var entries = [];
  var base = 'index:'+parts[2]+'/account';
  logger.debug("deviceMe",base,ids);
  Object.keys(ids).forEach(function(id){
    var index = {};
    index.idr = base + '#' + encodeURIComponent(id);
    index.account = parts[1];
    index.data = {};
  ...
  async.forEachLimit(Object.keys(pids), 5, function(pid, cbPids){
    ...
    async.forEach(Object.keys(ids), function(id, cbIds){
      ijod.getOne(base+'#'+encodeURIComponent(id), function(err, entry){
        ...
        // set up peering from this account to the other, pid is a localized id@devices
        friendPeer(app, account, entry.account, pid, function(isNew){
          // importantly, make sure reverse is set too!
          var viaPid = encodeURIComponent(idr.parse(entry.via).hash) + '@devices'; // use their localized device id for me
...
function friendPeer(app, src, dest, pid, cbDone)
{
  ...
  ijod.getOnePars(id, "ids", function(err, one) {
    ...
    ijod.setOneCat(id, "ids", {pars:pars}, function(){ cbDone(true) });
  });
...
exports.baseMerge = function(bases, options, callback)
{
  ...
  async.forEach(bases, function(base, cbBase){
    ...
    if(options.q) friends = friends.filter(function(friend){ return friend.matched });
    if(options.bio) friends = friends.filter(function(friend){ return friend.bio });

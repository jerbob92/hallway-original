...
//    if(entry._v !== VERSION || !entry.saved) return;
  ...
  async.forEach(Object.keys(pids), function(pid, cbPid){
    // first do the additional indexing on each friend
    friendex(pid, pids[pid], function(){
      ...
      async.forEach(Object.keys(auth.apps), function(app, cbApp){
        ...
        dal.query(sql, [app], function(err, rows) {
          ...
          logger.debug("found pairs",pid,app,pairs);
          async.forEachLimit(pairs, 10, function(pair, cbPair){
            ...
            friendPeer(app, pair.src, pair.dest, pair.pid, function(isNew){
              ...
              friendPeer(app, pair.dest, pair.src, pid, function(){
...
function friendPeer(app, src, dest, pid, cbDone)
{
  ...
  ijod.getOnePars(id, "ids", function(err, pars) {
    if(parStatus(pars, "peer")) return cbDone(false);
    // new peering!
    logger.debug("new peering found ",app,src,dest);
...
function friendex(primary, pids, cbDone)
{
  logger.debug("friendex",primary,Object.keys(pids));
  return cbDone();
}
...
{
  var parts = pid.split('@');
  return parts2par([dMap.partype(parts[1]), str2num(parts[0], 3)]);
}

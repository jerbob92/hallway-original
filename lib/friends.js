  ...
  async.forEach(bases, function(base, cbBase){
    ...
    ijod.getPars(base, options, function(err, pars){
      ...
      async.forEachLimit(Object.keys(pars), 10, function(idh, cbPars){
        ...
        outers.push(parInc(parSelect(pars[idh].pars, 'interactions'), iact.inter[id.hash]));
        outers.push(parInc(parSelect(pars[idh].pars, 'activity'), iact.act[id.hash]));
        outers.push(parInc(parSelect(pars[idh].pars, 'photos'), iact.photos[id.hash]));
        // TODO geo latlng

...
exports.bump = function(cset, auth, cbDone) {
  ...
  cset.forEach(function(entry) {
    ...
    if (participants.length > 0) {
      self = idr.parse(entry.idr).auth;
      // index of everyone we have interactions with
      if (participants.length > 1 && participants.indexOf(self) >= 0) {
  ...
  async.waterfall([
    function(cb) {
      indexMe(auth, false, cb); // XXX THIS DOES NOTHING
    },
    function(cb) {
...
exports.profile = function(profile, entry, light) {
  ...
  Object.keys(oembed).forEach(function(key) {
    ...
    if (light && profile[key] === oembed[key]) delete oembed[key];
  });

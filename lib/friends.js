...
exports.bump = function(cset, auth, cbDone) {
  ...
  cset.forEach(function(entry){
    ...
    { // below we do a lot more work on contacts, including self
      ...
      pids[dest] = entry;
      // TODO: for normal non-device contacts we should be looking for matching phone/email's too, as they may not be friends on the same network but across networks and still the same person
    }
  ...
  async.waterfall([
    function(cb){ indexMe(auth, self, cb); },
...
function selfIds(data)
{
  var ids = {};
  if(data.phone) ids[phoneHome(data.phone)] = true;
  if(data.email) ids[data.email.toLowerCase()] = true;
  if(Array.isArray(data.phones)) data.phones.forEach(function(phone){ ids[phoneHome(phone)] = true });
  if(Array.isArray(data.emails)) data.emails.forEach(function(email){ ids[email.toLowerCase()] = true });
  return ids;
}
...
function indexMe(auth, entry, cbDone)
{
  ...
  // first get all the unique ids, the device information is more rich than oembed (plurals), use that if so
  var ids = selfIds(isDevice ? entry.data : dMap.get('oembed', entry, entry.idr) || {});

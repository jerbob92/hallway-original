...
function interactive(auth, iact, cbDone)
{
  ...
  async.forEach(bases, function(base, cbBase){
    ijod.getPars(bases, options, function(err, pars){
      if(err) logger.warn("pars error",bases,err);
      if(!pars) return cbBase();

      async.forEachLimit(Object.keys(pars), 10, function(id, cbPars){
        if(pars[id].length == 0) return cbPars(); // skip non-indexed entries
        // get any existing values to increment
        var uid = all[id];
        var outers = []; // all updated outer pars
        outers.push(parInc(parSelect(pars[id], 'inter'), iact.inter[uid]));
        outers.push(parInc(parSelect(pars[id], 'act'), iact.act[uid]));
        outers.push(parInc(parSelect(pars[id], 'photos'), iact.photos[uid]));
        // TODO geo latlng
        ijod.setOnePars(id, "outer", outers, cbPars);
      });
    });
  }, cbDone);
}

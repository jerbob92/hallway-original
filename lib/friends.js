...
exports.bump = function(cset, auth, cbDone) {
  ...
  cset.forEach(function(entry){
    ...
    pids[idr.pid(entry.idr)] = entry;
  });
  
  if(pids.length == 0) return cbDone(null, cset);
  ...
  async.forEach(Object.keys(auth.apps), function(app, cbApp){
    var sql = "SELECT account, pid from Accounts where app = ? and profile in ("+ Object.keys(pids).map(function(pid){ return "'"+pid+"'"}).join(",") +")";
    dal.query(sql, [app], function(err, rows) {
      if(!rows || rows.length == 0) return cbApp();
      async.forEachLimit(rows, function(row, cbRow){
        // get our status par
          // if 
        // save to peers list, check if on invited for that pid and switch theirs to friended, save ours to friendly
        // do any notifications in async
      }, cbApp);
    });
...
// parallels are groupd into categories, since they're stored 4-per-row (for now, bit of a hack to fit current data model)

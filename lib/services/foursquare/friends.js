...
exports.syncFriends = function(auth, callback) {
  // TODO: add paging and don't just limit at 500, also any way to detect which
  // ones changed?
  request.get({
    uri: 'https://api.foursquare.com/v2/users/self/friends.json' +
         '?oauth_token=' + auth.accessToken + '&limit=500',
    json:true
  ...
  (function downloadUser() {
    ...
    request.get({
      ...
      (function parseUser() {
        ...
        if (friend === undefined ||
            friend.response === undefined ||
            friend.response.user === undefined) {
          return downloadUser();
        }

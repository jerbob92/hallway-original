...
exports.syncBadges = function (auth, callback) {
  getBadges(auth.accessToken, function(err, resp, js) {
    ...
    if(!js || !js.response || !js.response.badges) {
      return callback(
        new Error("missing response.badges: " + util.inspect(js))
      );
    }
    ...
    for (var badge in badges_json) {
      ...
      if (badges_json[badge].unlocks.length > 0) {
        newBadges.push(badges_json[badge]);
      }
    }
...
function getBadges(token, callback) {
  request.get({
    uri: 'https://api.foursquare.com/v2/users/self/badges.json?' +
         'v=20111202&oauth_token=' + token,
    json: true
  }, callback);
}

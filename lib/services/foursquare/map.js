...
exports.checkin = {
  ...
  oembed: function(data) {
    ...
    ret.lat = loc.lat;
    ret.lng = loc.lng;
    ret.title = (data.venue && data.venue.name) || (data.location && data.location.name);
    ret.url = data.url;
    ret.provider_name = 'foursquare';
...
exports.guid = {
  'checkin': function(entry) {
    ...
    if(entry.data.source && entry.data.source.name == "Instagram" && entry.data.shout && entry.data.shout.length > 0) guids.push('caption:instagram/#'+crypto.createHash('md5').update(entry.data.shout).digest('hex'));
    return guids.join(' ');
  },
  'photo': function(entry) {
    if(!entry.data.checkin) return undefined;

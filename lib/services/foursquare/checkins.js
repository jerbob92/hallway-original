...
function getCheckins(config, userID, token, offset, callback, checkins) {
  ...
  request.get({
    uri:'https://api.foursquare.com/v2/users/self/checkins.json' +
        '?limit=' + checkins_limit + '&offset=' + offset +
  ...
  function(err, resp, js) {
    ...
    if (!js || !js.response || !js.response.checkins) {
      return callback(
        new Error("missing response.checkins: " + util.inspect(js))
      );
    }
    var response = js.response;
    if (!(response.checkins && response.checkins.items)) { //we got nothing
      if (checkins.length > 0) config.checkinsThrough = checkins[0].createdAt;
...
function getMe(token, callback) {
  request.get({
    uri:'https://api.foursquare.com/v2/users/self.json?oauth_token=' + token
  }, callback);
}

function addAll(thisArray, anotherArray) {
  if (!(thisArray && anotherArray && anotherArray.length)) return;

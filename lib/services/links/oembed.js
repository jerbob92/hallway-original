...
function refcheck(refs) {
  ...
  for(var i = 0; i < refa.length; i++) {
    if (refa[i].indexOf('link:') === 0) return true;
  }
  return false;
}
...
exports.pump = function(cset, callback) {
  ...
  function saver(task, data) {
    ...
    task.entries.forEach(function(entry){
      ...
      entry.q = [entry.q,data.title,(data.type==="link")?data.url:''].join(" ");
    });
  }
...
//        if (!data) data = {err:'Could not fetch oEmbed data.'};
  ...
  cset.forEach(function(entry){
    ...
    Object.keys(entry.refs).forEach(function(url){
      ...
      var digest = crypto.createHash('md5').update(task.url).digest('hex');
      task.idr = idr.parse('oembed:links/oembed#' + digest);
...
function oembed(url, callback) {
  ...
  async.forEachSeries(stack, function (u, cb) {
    if (!u) return process.nextTick(cb);

...
function refcheck(refs) {
  ...
  for(var i = 0; i < refa.length; i++) {
    if (refa[i].indexOf('http') === 0) return true;
  }
  return false;
}
...
exports.pump = function(cset, callback) {
  ...
  var q = async.queue(function(task, cb){
    ...
    expand({url:task.url}, function(arg){
      ...
      // use original if expansion failed
      if (typeof arg.url !== 'string') arg.url = task.url;
      saver(task, arg.url);
      process.nextTick(cb);
    });
  }, 10);
...
var APIs = {
  ...
  // all of these try to recurse on any result, or any error fall back to
  // generic HEAD request

  isgd: function (args, callback) {
    var url = 'http://is.gd/forward.php?' + querystring.stringify({
      format: 'json',
      shorturl: args.urlp.pathname.replace('/', '')
    });
  ...
  budurl: function (args, callback) {
    var url = 'http://budurl.com/api/v1/budurls/expand?' +
              querystring.stringify({
                budurl: args.urlp.pathname.replace('/', '')
              });
  ...
  snipurl: function (args, callback) {
    var url = 'http://snipurl.com/resolveurl?' +
              querystring.stringify({
                id: args.urlp.pathname.replace('/', '')
              });
  ...
  generic: function (args, callback) {
    ...
    // t.co returns meta refresh if browser!
    if (args.urlp.host === "t.co") headers = {};
    if (args.same && args.headers && args.headers['set-cookie']) {
      // really dumb hack to enable cookie-tracking redirectors
      headers.Cookie = args.headers['set-cookie'];
    }
    // workaround to fix Parser Error's after the request,
    ...
    var req = request.head({
      ...
      if (res.statusCode === 301 ||
        ...
        if (!res.headers.location) {
          args.err = 'missing location header on a 3xx';
          return callback(args);
        }
        var newup = urllib.parse(
          urllib.resolve(args.urlp,urllib.parse(res.headers.location))
        );
        // if the url is unparseable, bail out
        if (!newup || !newup.pathname) return callback(args);
        // if we're redirected to a login page, bail, kinda lame heuristic here
        // but it works pretty well!
        if (newup.pathname.indexOf("login") > 0 &&
            newup.pathname.indexOf("login") < 10) {
          return callback(args);
        }
     ...
     // this is double-erroring due to or related to
...
// this was generated from http://api.longurl.org/v2/services?format=json on
// 7/31/12 and should be updated if you are reading this over a month past!
var SHORTs = {
  "0rz.tw": {
    "domain": "0rz.tw",
    "regex": ""

...
module.exports = {
  handler : function (callback, apiKeys, done, req, res) {
    ...
    if (qs && qs.contextio_token) {
      var path = 'connect_tokens/' + qs.contextio_token;
      lib.fetch(apiKeys, path, function(err, js){
        if (err) return done(err);
        if (!js || !js.account || !js.account.id) {
          return done(new Error("response was missing account"));
        }
        done(null, {
    ...
    lib.fetch(apiKeys, 'connect_tokens', function(err, js){
      if (err) return done(err);
      if (!js || !js.browser_redirect_url) {
        return done(new Error("missing generated redir url"));
      }

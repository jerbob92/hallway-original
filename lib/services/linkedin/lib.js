...
function getCachedProfile(id, appID, minTimestamp, callback) {
  ...
  ijod.getOne(idr, function(err, obj) {
    ...
    logger.debug('app %d, got linkedin profile from cache: %s', appID, id);
    return callback(null, obj.data);
  });
}

function setCachedProfile(id, appID, profile, timestamp, callback) {
  var idr = 'profile:' + appID + '/linkedin#' + id;
  var obj = {
    idr: idr,
    data: profile,
  ...
  ijod.batchSmartAdd([obj], callback);
}

exports.getConnection = function(id, auth, updateTimestamp, callback) {
  var appID = Object.keys(auth.apps)[0];
    ...
    exports.get(path, auth, function(err, profile) {
      if (!err) {
        ...
        process.nextTick(function() {
          setCachedProfile(id, appID, profile, updateTimestamp, function(err) {
            if (err) {

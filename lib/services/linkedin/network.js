...
exports.sync = lib.genericSync(function(pi) {
  ...
  var newestConnectionSynced = Math.max(networkConnUpdate,
                                        pi.config.connLastModified);
  ...
  getConnections(Object.keys(updatedConnIDs), pi.auth, function(errs, conns) {
    if (errs) {
...
function getConnections(ids, auth, cbDone) {
  ...
  async.forEachLimit(ids, 25, function(id, cbEach) {
    lib.getConnection(id, auth, function(err, connection) {
      if (err) errs[id] = err;

...
// SYNC_KEY contains object describing the current state of the synclet:
...
exports.dailySync = function(apiPath, item, idrType, idrPath, deviceType) {
  ...
  return this.genericSync(function(pi) {
    ...
    if (!sync.begin) sync.begin = new Date(pi.config.memberSince).getTime();
    ...
    var newConfig = {};
    var sync = newConfig[SYNC_KEY] = pi.config[SYNC_KEY];
    sync.hits++;

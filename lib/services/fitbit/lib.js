...
exports.dailySync = function(apiPath, item, idrType, idrPath, deviceType) {
  ...
  return this.genericSync(function(pi) {
    ...
    if (!all.self || !all.devices) {
      ...
      if (!pi.config.waitingRetries) pi.config.waitingRetries = 0;
      pi.config.waitingRetries++;

      switch (pi.config.waitingRetries) {
        case 1:
          pi.config.nextRun = 30; // Hope they're running right now
          break;
        case 2:
          pi.config.nextRun = 300; // Give it more time
          break;
        default:
      ...
      delete pi.config.waitingRetries;
    }

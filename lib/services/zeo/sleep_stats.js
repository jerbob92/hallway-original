...
function syncSleepStats(statsArray, pi, cb) {
  if (statsArray.length >= MAX_QUERIES) pi.config.nextRun = -1;
  pi.data = {};
  if (statsArray.length > 0) pi.config.since = statsArray[statsArray.length-1].startDate;
...
exports.sync = function(pi, cb) {
  ...
  lib.apiCall({ auth: pi.auth, query: '/getEarliestSleepStats' },
    ...
    var statsArray = [];
    if (pi.config.since)  {
      stat.startDate = pi.config.since;
      console.log('starting from since');
    }
    else statsArray.push(stat);

...
{
  var arg = {};
  arg.oauth = {consumer_key:pi.auth.consumerKey, consumer_secret:pi.auth.consumerSecret, token:pi.auth.token, token_secret:pi.auth.tokenSecret};
  arg.data = {};
  ...
  delta(arg, function(err){
    cb(err, {data:arg.data, config:{cursor:arg.form.cursor}});
  });
}
...
function delta(arg, cbDone)
{
  request.post(arg, function(err, res, deltas){
    ...
    deltas.entries.forEach(function(entry){
      if(!entry[1]) return;
      (entry[1].is_dir === true) ? arg.folders.push(entry[1]) : arg.files.push(entry[1]);
    });
    arg.form.cursor = deltas.cursor;
    if(deltas.has_more === true) return delta(arg, cbDone);
    cbDone();
  });
}

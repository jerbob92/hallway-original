...
{
  var OA = new OAlib(null, 'https://api.bodymedia.com/oauth/access_token?api_key='+auth.consumerKey, auth.consumerKey, auth.consumerSecret, '1.0', null, 'HMAC-SHA1', null, {'Accept': '*/*', 'Connection': 'close'});
  var url = 'http://api.bodymedia.com/v2'+req.url+'?api_key='+auth.consumerKey;
  var qs = querystring.stringify(req.query);
  if (qs) url += '&'+qs;
  // only supporting GET at the moment
  OA.get(url, auth.token, auth.tokenSecret, function(err, data, response){
    if(err && err.statusCode == 401 && !flag) return refreshToken(OA, auth, req, res);
...
{
  oauth.getOAuthAccessToken(auth.token, auth.tokenSecret, function (error, oauth_token, oauth_token_secret, additionalParameters) {

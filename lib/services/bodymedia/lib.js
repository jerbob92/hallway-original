...
exports.genericSync = function(type, pather, arrayer) {
  return function f(pi, cb, flag) {
    ...
    OA.get(url, pi.auth.token, pi.auth.tokenSecret, function(err, body){
      ...
      if(err && err.statusCode == 403 && !flag) return setTimeout(function(){f(pi, cb, true)}, Math.random()*20000); // default noncom is enforced at 2/sec (https://developer.bodymedia.com/forum/read/156586) one chance to wait just a bit and try again for lack of better way of doing this

...
exports.sync = function(pi, callback) {
  ...
  conn.connect(function(error) {
    ...
    conn.openBox("[Gmail]/All Mail", true, function(error, box) {
      ...
      var lastSeenUID = 1;
      if (pi.config && pi.config.uid_validity && box.uidvalidity && pi.config.uid_validity == box.uidvalidity) {
        // Get more!
          ...
          cb: function (fetch) {
            ...
            fetch.on("end", function() {
              conn.logout(function() {
                ...
                contacts.findContacts({auth:pi.auth, config:pi.config}, msgInfo, function(error, results) {
                  var config = {lastSeenUID:maxPullID, uid_validity:box.uidvalidity};
                  if (maxPullID < box.uidnext) config.nextRun = -1;
                  var data = {};
                  // Merge in the contacts data and config if it finished properly
                  if (!error) {
                    if (results.data) Object.keys(results.data).forEach(function(key) {
                      data[key] = results.data[key];
                    });
                    if (results.config) Object.keys(results.config).forEach(function(key) {

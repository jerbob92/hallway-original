...
exports.sync = function(pi, cbDone) {
  request.get({uri:'https://www.googleapis.com/oauth2/v1/userinfo', headers:{authorization:'Bearer '+pi.auth.token.access_token}, json:true}, function(err, resp, me) {
    ...
    if(resp.statusCode != 200 || !me || !me.id) return cbDone(new Error(resp.statusCode+': '+JSON.stringify(me)))
    console.log("%j", me);
    ...
    conn.connect(function(error) {
      ...
      conn.getBoxes(function(error, boxes) {
        ...
        conn.logout(function() {
          var data = {};
          data["profile:" + pi.auth.pid + "/self"] = [{id:emailAddy, boxes:allBoxes, profile:me}];

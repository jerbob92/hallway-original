...
module.exports = {
  handler : function (callbackURL, apiKeys, done, req, res) {
    ...
    var sess;
    if(req.cookies && req.cookies["rdio_client"]) try { sess = serializer.parse(req.cookies["rdio_client"]) }catch(E){}
    ...
    oa.getOAuthRequestToken( { oauth_callback : callbackURL}, function (error, oauth_token, oauth_token_secret, oauth_authorize_url, additionalParameters) {
      if (error) return res.end("failed to get token: " + error);
      res.cookie('rdio_client', serializer.stringify({token_secret:oauth_token_secret}), { path: '/', httpOnly: false }); // stash the secret

...
exports.getPage = function(pi, endpoint, type, perPage, params, callback) {
    ...
    if (!config[type].paging) config[type].paging = {};
    if(!config[type].paging.lastPage)
        config[type].paging.lastPage = 0;
    var thisPage = config[type].paging.lastPage + 1;
    ...
    oa.getProtectedResource('http://api.flickr.com/services/rest/?' + qs.stringify(params),
          ...
          if(page > pages) { //whoa whoa whoa, bad news bears, page should never be greater than pages
              ...
              config[type].paging.lastPage = 0;
              config[type].paging.totalPages = -1;
              ...
              config[type].paging.lastPage = page;
              config[type].paging.totalPages = pages;
              config.nextRun = -1;
          }

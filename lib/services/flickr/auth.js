module.exports = {
    handler : function (callback, apiKeys, done, req, res) {
        ...
        var sess;
        if(req.cookies && req.cookies["flickr_client"]) try { sess = serializer.parse(req.cookies["flickr_client"]) }catch(E){}
        ...
        OA.getOAuthRequestToken( { oauth_callback: callback }, function (error, oauth_token, oauth_token_secret, oauth_authorize_url, additionalParameters) {
            ...
            res.cookie('flickr_client', serializer.stringify({token_secret:oauth_token_secret}), { path: '/', httpOnly: false }); // stash the secret

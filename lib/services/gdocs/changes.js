...
exports.sync = function(pi, cb) {
  ...
  page(arg, function(err){
    cb(err, {data : data, auth: pi.auth, config:{changestamp:arg.newest}});
  });
};

function page(arg, callback)
{
  var url = "https://docs.google.com/feeds/default/private/changes?alt=json&access_token="+arg.auth.token.access_token+"&v=3&start-index="+(arg.newest+1);
  lib.get(arg.auth, {uri:url, json:true}, function(err, resp, body){
    ...
    body.feed.entry.forEach(function(e){
      if(e.docs$changestamp && e.docs$changestamp.value && e.docs$changestamp.value > arg.newest) arg.newest = parseInt(e.docs$changestamp.value); // track the newest
      arg.changes.push(e);
    });

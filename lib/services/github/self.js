...
exports.sync = function(pi, cb) {
  request.get({url:"https://api.github.com/user?access_token="
      + pi.auth.accessToken, json:true}, function(err, resp, body) {
    if(err || !body) return cb(err);
    if(!body.id) {
      if (body.message) return cb('error from github: ' + body.message);
      return cb(new Error("invalid response from github: "+JSON.stringify(body)));
    }

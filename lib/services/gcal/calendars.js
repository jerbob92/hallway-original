...
{
  var url = "https://www.googleapis.com/calendar/v3/users/me/calendarList?key="+arg.auth.appKey;
  if(arg.pageToken) url += "&pageToken="+arg.pageToken;
  lib.get(arg.auth, {uri:url}, function(err, resp, body){
    if(err || !body || !body.items || !Array.isArray(body.items) || body.items.length == 0) return callback(err);

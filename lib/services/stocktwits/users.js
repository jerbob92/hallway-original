...
exports.sync = function(processInfo, cb) {
  ...
  arg.headers = {"Authorization":"OAuth "+processInfo.auth.accessToken, "Connection":"keep-alive"};
...
{
  request.get({url:"https://api.stocktwits.com/api/2/graph/"+type+".json?limit=500", json:true, headers:arg.headers}, function(err, resp, body) {
    if(err || !body || !Array.isArray(body.users)) return cb(err);
    arg.types[type] = body.users; // TODO: pagination, not quite sure how it works yet
    cb();
  });
}

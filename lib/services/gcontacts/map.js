...
exports.contact = {
  ...
  oembed: function(data) {
    ...
    if(Array.isArray(data.link)) for(var i = 0; i < data.link.length; i++)
    {
      ...
      if(link.rel.indexOf('#photo') > 0 && link.gd$etag) ret.thumbnail_note = "https://api.singly.com/id/ENTRYID?media=true&access_token=TOKEN";
    };
    return ret;
  },
  text: function(data) { return Array.isArray(data.gd$email) && data.gd$email[0].address }
}

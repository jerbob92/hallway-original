...
exports.guid = {
  'photo': function(entry) {
    ...
    var guids = [];
    var match;
    if((match = /instagr.am\/p\/([^\/]+)\//.exec( entry.data.link ))) guids.push('guid:instagram/#'+match[1]);
    if(entry.data.caption && entry.data.caption.text && entry.data.caption.text.length > 0) guids.push('caption:instagram/#'+crypto.createHash('md5').update(entry.data.caption.text).digest('hex'));

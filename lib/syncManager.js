...
var NUM_WORKERS = lconfig.syncManager.numWorkers;
var PAGING_TIMING = lconfig.syncManager.pagingTiming;
var DEFAULT_SCAN_TIME = lconfig.syncManager.defaultScanTime;
...
function errorHelper(text, callback, opt_returnError) {
  if (opt_returnError === undefined) {
    opt_returnError = true;
  }
...
SyncletManager.prototype.backfillQueue = function(callback) {
  ...
  self.skew.isFilled(function(err, filled) {
    if (filled) {
      logger.info('The queue was filled, not performing backfill.');
    ...
    self.skew.getFillLock(function(err, gotLock) {
      ...
      async.until(
        ...
        function(err) {
          ...
          self.skew.setFilled(function(err) {
...
SyncletManager.prototype.schedule = function(task, timeToRun, cbDone) {
  ...
  self.skew.isJobRunning(self.getKey(task), function(err, running) {
    ...
    if (running) {
      return errorHelper("Attempted to reschedule a synclet while it's running", cbDone, false);
    }
    ...
    self.skew.schedule(key, timeToRun, task, DEFAULT_PRIORITY, function(err, jobId) {
      if (err || !jobId) {
      ...
      instruments.increment("synclet.schedule").send();
  ...
  async.whilst(
    ...
    function(callback) {
      self.skew.reserve(function(err, jobId, task) {
        if (err || !jobId || !task) {
          logger.info("Error reserving a job: " + err);

          return callback(err);
        }

        instruments.increment("synclet.reserve").send();
...
SyncletManager.prototype.runTask = function(task, callback) {
      ...
      if (task.count > 0 || !err) {
        ...
        self.skew.destroy(self.getKey(task), task.jobId, function(destroyErr) {
          if (destroyErr) {
            ...
            instruments.increment("synclet.destroy").send();
          }

          self.completed(response, task, runInfo, callback);
        });
      } else {
  ...
  async.series([
    ...
    try {
      ...
      // This should never be a double-callback!
      cbDone(E);
    }
  });
};

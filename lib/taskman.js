...
var STATS = {total:0, now:Date.now(), last:[]};
var SERVICES = {};
var SYNCLETS = {};
var WORKERS = {};
var TIMERS = {};
  ...
  exports.loadSynclets(function(){        
    ...
    redis.select(1, function(){
      ...
      if(live)
      {
        ...
        TIMERS.fill = setInterval(fillNext, DEFAULT_SCAN_TIME);
...
exports.stop = function(callback)
{
  ...
  setTimeout(function(){stop(callback)}, 10000);
}
...
{
  STATS.workers = Object.keys(WORKERS).length;
  return STATS;
}
...
exports.backlog = function(callback)
{
  ...
  async.forEachSeries(Object.keys(SERVICES), function(service, cbSvcLoop){
    async.forEach(Object.keys(SYNCLETS[service]), function(synclet, cbSyncLoop){
      ijod.getBounds('task:'+service+'/'+synclet, {}, function(err, bounds){
    ...
    callback({total:total, oldest:oldest});
  });
}
...
function fillNext()
{
  // only be running one at a time
  if(STATS.filllock) {
    logger.warn("fill locked for",Math.floor((Date.now() - STATS.filllock)/1000));
    return;
  }
  STATS.filllock = Date.now();
  
  // try to scan into the future when not getting any data
  STATS.now = (STATS.lastfill) ? Date.now() : STATS.now + DEFAULT_SCAN_TIME;
  STATS.lastfill = false;
  
  var pids = {};
  async.forEachSeries(Object.keys(SERVICES), function(service, cbSvcLoop){
    async.forEach(Object.keys(SYNCLETS[service]), function(synclet, cbSyncLoop){
      // just get the 100 oldest... maybe use special db-only to get list of idrs and getOne each would be faster?
      ijod.getRange('task:'+service+'/'+synclet, {until:STATS.now, reverse:true, limit:100}, function(task){
        if(task.at > STATS.now) return; // ignore edge case happens when debugging, memcache'd one is newer than saved
        logger.debug('aged task',task.idr,task.at);
        pids[task.pid] = true;
      }, cbSyncLoop);
    }, cbSvcLoop);
  }, function(){
    delete STATS.filllock;
    if(Object.keys(pids).length == 0) return;
    
    // we've got tasks!
    STATS.lastfill = true;
    STATS.now = Date.now();
    logger.debug("filling",Object.keys(pids));
    Object.keys(pids).forEach(function(pid){
      if(Object.keys(WORKERS).length > NUM_WORKERS) return redis.sadd("next", pid); // dump into next queue for others if already max busy
      isLocked(pid, function(locked){
        if(locked) return;
        runWorker(pid, function(){});
      });      
...
{
  if(ONEPROFILE && pid != ONEPROFILE) return callback(true); // convenient way to lock to just one profile
...
exports.taskUpdate = function(auth, callback)
{
  ...
  getTasks(auth.pid, function(curtasks){
    ...
    async.forEach(livesynclets, function(synclet, cbLoop){
      ...
      added = true;
      var task = {idr:taskid, at:Date.now(), pid:auth.pid, created:Date.now(), service:service, synclet:synclet};
...
function runTask(pi, task, callback)
{
  ...
  function cbDone(err, response) {
    ...
    pipeline.inject(response.data, pi.auth, function(err) {
      ...
      if(typeof response.config == 'object') profileManager.configSet(pi.auth.pid, response.config, function(){});
...
function runWorker(pid, callback, force)
{
  ...
  redis.hsetnx("active",pid,[Date.now(),WORKER_NAME].join("\t"), function(err, set){
    ...
    profileManager.allGet(pid, function(err, pi) {
      if(!pi || !pi.auth) return cbDone(new Error("missing profile info"));
      ...
      getTasks(pid, function(tasks){
        ...
        async.forEach(Object.keys(tasks), function(taskid, cbLoop){
          ...
          async.forEachSeries(Object.keys(todo), function(taskid, cbLoop){
            ...
            clearInterval(self.hbtimer); // just in case do it preemptively here too
            redis.hdel("active",pid, function(){
              checkNext(); // more capacity now
              cbDone(null, {tasks:todo, pi:pi});
            });
          });          
        });
      });

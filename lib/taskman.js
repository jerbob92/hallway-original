  ...
  redis.select(1, function() {
    locksmith.init(WORKER_NAME, function(){
      queenBee.init(function() {
        ...
        // role where we receive pids to do from a parent process
        if (live === "child") {

          // tell them we're alive yet!
          setInterval(function(){
            process.send({"heartbeat":true});
          }, lconfig.taskman.heartbeat || 10000);

          // they gave us a pid to work on
          process.on('message', function(todo) {
            logger.debug('CHILD got todo', todo);
            runWorker(todo.pid, function(err){
              if (err) todo.err = err.toString();
              logger.debug('CHILD finished todo', todo);
              // let em know it's done
              process.send(todo);
            });
          });

          // call child services! parent abandonment!
          process.on('disconnect', function() {
            logger.info('parent disconnected');

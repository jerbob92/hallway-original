...
//        logger.debug('aged task',task.idr,task.at);
    ...
    logger.debug("fillNext took",parseInt((Date.now() - start)/1000),plist.length);
...
function saveTask(task, callback)
{
  ...
  redis.get(task.idr, function(err, deleted){
    ...
    task.at = parseInt(Date.now() + nextRun);
  ...
  redis.hsetnx("active",pid,[Date.now(),WORKER_NAME].join("\t"), function(err, set){
    ...
    profileManager.allGet(pid, function(err, pi) {
      ...
      getTasks(pid, function(tasks){
        logger.info("scanning tasks",Object.keys(tasks).map(function(id){return [id,tasks[id].at-Date.now()].join(" ") }).join(" "));
        ...
        async.forEach(Object.keys(tasks), function(taskid, cbLoop){
          ...
          if(todo.length === 0)
          {
            logger.warn("no tasks found to do!",pid,Object.keys(tasks).map(function(id){return [id,tasks[id].at].join(" ") }).join(" "));
            return cbDone();
          }
          self.tasks = todo;

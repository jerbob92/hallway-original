...
function isLocked(pid, callback) {
  ...
  redis.hget("active", pid, function(err, lock) {
    ...
    // TODO very small race condition here if someone else removed and re-added
    // in between
...
exports.taskUpdate = function(auth, callback, force) {
  ...
  async.forEach(apps, function(app, cbLoop){
    ...
    var idego = auth.apps && auth.apps["0d0dfc9344d5046e55d57ed001573793"];
    if(idego && Object.keys(auth.apps).length === 1) livesynclets = [];
    ...
    getTasks(auth.pid, function(curtasks) {
      ...
      async.forEach(livesynclets, function(synclet, cbLoop) {
        ...
        async.forEach(Object.keys(curtasks), function(taskid, cbLoop) {
          ...
          // if there's new tasks, def queue them up asap
...
function runTask(pi, task, fnCallback) {
  ...
  if (sandboxed) {
    ...
    _.forEach(orig, function(value, key) {
      if (key.toString()[0] === '_') pi.config[key] = value;
    });
  }
  ...
  function cbDone(err, response) {
    ...
    var callback = function(err) {
      ...
      if (sandboxed) {
        ...
        // move special objects to the top level
        _.forEach(cfg, function(value, key) {
          if (key.toString()[0] === '_') {
            configUpdate[key] = cfg[key];
            delete cfg[key];
          }
        });
      }
...
function runWorker(pid, callback, force) {
  ...
  redis.hsetnx("active", pid, lockData(), function(err, set) {
    ...
    self.hbtimer = setInterval(function() {
      ...
      redis.hget("active", pid, function(err, lock) {
        ...
        if (lock.indexOf(WORKER_NAME) === -1) {
          return self.killed = "lock stolen by " + lock;
        }
        redis.hset("active", pid, lockData());
      });
    }, LOCK_TIMEOUT / 2);

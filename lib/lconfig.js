...
exports.load = function(filepath) {
  ...
  exports.papertrailToken = config.papertrailToken || '';
  exports.papertrailGroup = config.papertrailGroup || 0;
  ...
  exports.taskman = config.taskman || { redis: { host: 'localhost', port: 6379 } };
  exports.authSecrets = config.authSecrets || {crypt:'foo', sign:'bar', oldcrypt:'foo', oldsign:'bar'}; // these need to be required to be set in prod, trusted cookies use them during auth
  exports.cookieExpire = config.cookieExpire || (60 * 60 * 24 * 30); // default 30 days
  if (exports.stats.prefix) {
    var hostname = process.env.HOSTNAME, hostBasename;

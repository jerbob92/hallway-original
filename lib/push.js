...
rclient.select(4, function(){});

var timeout = 15000;
var maxBacklog = 10;

// push used to be called webhooks but it appears that name isn't being used as
// much anymore, sometimes http notifications too, or subscriptions
...
exports.pump = function(cset, auth, cbDone) {
  ...
  getRoutes(auth, function(routes) {
    ...
    async.forEach(Object.keys(bases), function(base, cb) {
      ...
      // any catch-all globals
      if (routes['*']) routes['*'].forEach(function(route) {
        rts.push(route);
      });
      async.forEach(rts, function(route, cb2) {
        // skip bad routes
        if (!route.url || route.url.substr(0,4) !== "http") {
        ...
        set.forEach(function(entry) {
          ...
          if (typeof entry.oembed === 'string') {
            entry.oembed = oembeds[entry.oembed];
          }
        });
        logger.warn("pushing",base, set.length," entries to ",route);
        var args = {
          uri: route.url,
          timeout: timeout,
          body: '[' + set.map(function(entry) {
            return entries.toString(entry, route.options);
          }).join(',') + ']',
        ...
        request.post(args, function(err, resp, body) {
          ...
          if (is2xx(status) && (!route.backlog || route.backlog.length === 0)) {
            return cb2();
          }

          // optionally async into any backlog if the endpoint is working,
          // otherwise this passes through
          logger.debug("updating saved route entry");
          var backlog = (is2xx(status) && route.backlog) ? route.backlog : [];
          var stillbroke = [];
          async.forEach(backlog, function(id, cb3) {
            // fetch the stored backlog entry
            // TODO:  Is this only inside a pod?
            ijod.getOne(id, function(err, entry) {
              if (err || !entry) return cb3(); // is missing?!
              logger.debug("pushing backlog",entry.idr);
              args.body = '['+entry.data.map(function(entry) {
                return entries.toString(entry, route.options);
              }).join(',')+']';
              request.post(args, function(err, resp, body) {
                if (resp && is2xx(resp.statusCode)) return cb3();
            ...
            var add = [];
            // booooo something failed, save out the exact snapshot in time for
            // the backlog
            if (!is2xx(status) && status !== 410) {
            ...
            nexusClient.batchSmartAdd(add, function(err) {
              if (err) return cb2();
              // now update the original routes entry
              // everything else we're updating saved state, so always fetch the
              // newest to update, almost atomic :)
              nexusClient.getOne(route.idr, function(err, entry) {
                // get matching route
...
exports.firstRun = function(routes, profiles, cbDone) {
  ...
  async.forEach(Object.keys(routes), function(filter, cb) {
    ...
    // VALIDATE route.url! should go back to app and should def not reference
    // api.singly.com (infinite looping possibility)
    ...
    entries.runBases(bases, options, function(entry, base) {
      ...
      async.forEach(oembeds, function(entry, cb2) {
        podClient.getOne(entry.oembed, function(err, oembed) {
        ...
        if (oembeds.length > 0) set = set.filter(function(entry) {
          return typeof entry.oembed === 'object';
        });
        // got the entries, send em!
        // TODO this part should be refactored to a shared function with the
        // pump, and should we save the backlog?
...
function getRoutes(auth, cbDone)
{
  ...
  // go through all the apps auth'd to this profile and extract any possible
  // route bundles stored
  var idrs = {};
  Object.keys(auth.apps).forEach(function(app) {
    ...
    // give the app-wide one more buffer
    idrs['routes:'+app+'/push#default'] = maxBacklog*10;
  });

  // fetch all the possible routes (getOne is memcached, should always be
  // fast/cheap)
  logger.debug("looking for routes at",idrs);
  async.forEach(Object.keys(idrs), function(id, cb) {
    nexusClient.getOne(id, function(err, entry) {
      ...
      Object.keys(entry.data).forEach(function(filter) {
        ...
        // set above as maxBacklog threshold
        if (route.backlog && route.backlog.length > idrs[id]) {
          logger.warn("backlog too large, skipping route ",route);
          return;
        }
        // these are usually an api url to filter or * to match all data as the
        // special case
        if (filter === '*') {
          if (!routes['*']) routes['*'] = [];
          routes['*'].push(route);
        ...
        // process the filter as a url, add .options, routes[base] each possible
        // base
        var url = urllib.parse(filter, true);

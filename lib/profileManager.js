...
function loadProfile(id, cbDone) {
...
function createProfile(id, cbDone) {
  if (role === 'pod') return createProfileLocal(id, cbDone);
  else return createProfileRemote(growingPodID(), id, cbDone);
}

function createProfileLocal(id, cbDone) {
  var sql = 'INSERT INTO Profiles (id, service) values (?, ?)';
  var service = id.split('@')[1];
  dal.query(sql, [id, service], cbDone);
}

function createProfileRemote(pod, id, cbDone) {
  cbDone(new Error('TODO: Create on remote'));
}
...
function genGet(id, cbDone) {
  ...
  loadProfile(id, function(err, profile) {
    if (err) return cbDone(err);

    if (!profile) {
      return createProfile(id, function(err) {
        return cbDone(err, {
          auth   : {},
          config : {}
        });
    ...
    if (role === 'pod' || !profile.pod) {
      return genGetLocal(id, cbDone);
    } else {
      return genGetRemote(profile.pod, id, cbDone);
    }
  });
}
exports.allGet = genGet;

function genGetRemote(pod, id, cbDone) {
  logger.debug(role, 'Getting remote profile', pod, id);
  podClient.getProfileData(pod, id, cbDone);
}

function genGetLocal(id, cbDone) {
  logger.debug('Getting local profile', id);

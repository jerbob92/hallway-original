...
function getAll(base, sql, binds, cbDone)
{
  var ndx = {};
  var ret = [];
  partition.readFrom(base, function(parts){
    async.forEach(parts, function(part, cbParts){
      part.dal.query(sql.replace('T4BL3',part.table), binds, function(err, rows){
        if(err) logger.warn("getAll query failed",sql.replace('T4BL3',part.table), binds, err);
        if(rows) rows.forEach(function(row){
          if(ndx[row.idr]) { // if this idr exists, see if we have to replace it
            if(part.trump) rows.splice(ndx[row.idr],1,row);
            return;
          }
          ndx[row.idr] = rows.length; // track it's offset for possible replacement
          rows.push(row);
        });
        cbParts();
      });
...
function getMany(base, sql, binds, sort, limit, cbDone)
{
  getAll(base, sql, binds, function(err, rows){
    if(err) return cbDone(err);
    if(sort) rows.sort(sort);
    cbDone(null, rows.slice(0,limit));
  });
}

// just try to get one in order
function getOne(id, sql, binds, cbDone)
{
  var ret = false;
  partition.readFrom(id, function(parts){
    async.until(function(){ return ret !== false; }, function(cbUntil){
      ...
      part = parts.pop();
      part.dal.query(sql.replace('T4BL3',part.table), binds, function(err, rows){
        if(err) logger.warn("getOne query failed",sql.replace('T4BL3',part.table), binds, err);
...
// returns all rows but validates by idr (required in row)
function inserter(id, sql, binds, cbDone)
{
  partition.writeTo(id, function(parts){
    async.forEach(parts, function(part, cbParts){
      part.dal.query(sql.replace('T4BL3',part.table), binds, function(err, rows){
        if(err) logger.warn("inserter query failed",sql.replace('T4BL3',part.table), binds, err);
        cbParts();
      });
    }, cbDone);
  });
...
exports.getPars = function(basePath, options, cbDone) {
  ...
  getAll(basePath, sql, binds, function(err, rows) {
    if (err) return cbDone(err);
...
exports.getOnePars = function(targetIdr, cat, cbDone) {
  ...
  var sql = "SELECT lpad(hex(par),32,'0') as par, path, hex(idr) FROM Entries WHERE idr in ("+ entryInClause +")";
  if(idp.table != "Entries") sql = unionize(idp.table.split('_')[1], sql, []); // when we can, we need to union this search
  logger.debug(sql);
  getAll(targetIdr, sql, [], function(err, rows) {
    if (err) return cbDone(err);
    if (rows.length === 0) return cbDone(null);
...
IJOD.prototype.addData = function(arg, callback) {
  ...
  var sql = "INSERT INTO "+ tableize(idr.partition(arg.idr)) +" (base, idr, path, hash, offset, len, lat, " +
...
exports.getOne = function(targetIdr, callback) {
  ...
  var idp = idParse(targetIdr);
  memcache.get(idp.hash, function(error, result) {
    var js;
    try {
      js = JSON.parse(result[idp.hash]);
    } catch(E) {}
    if (error || result === "NOT_STORED" || result === null || !js) {
      var sql = "SELECT path, offset, len FROM Entries WHERE idr = x? LIMIT 1";
      var binds = [idp.hash];
      if(idp.table != "Entries") sql = unionize(idp.table.split('_')[1], sql, binds); // when we can, we need to union this search
      dal.query(sql, binds, function(error, rows) {
        ...
        backend.get(rows[0].path, rows[0].offset, buf.length, function (err, buf) {
          ...
          memcache.set(idp.hash, jsonStr, function(error, result) {
...
exports.delOne = function(targetIdr, callback) {
  ...
  if (typeof targetIdr !== 'string') return callback("invalid id: "+targetIdr);
  var idp = idParse(targetIdr);
  logger.debug("deleting",idp);
  memcache.del(idp.hash, function(error, result) {
    dal.query("DELETE FROM Entries WHERE idr = x? LIMIT 1", [idp.hash], function(error) {
      if (idp.table == "Entries") return callback();
      // also delete from partition
      dal.query("DELETE FROM "+idp.table+" WHERE idr = x? LIMIT 1", [idp.hash], function(error) {
        callback();
      });
    });
  });
...
exports.getRange = function(basePath, range, cbEach, cbDone) {
  ...
  var self = this;
  var sql = "SELECT path, offset, len, base FROM Entries WHERE base > " +
  ...
  sql = unionize(idr.partition(basePath), sql, binds);
  if (exports.debug) {
...
exports.batchSmartAdd = function(entries, callback) {
  ...
  var partition = idr.partition(entries[0].idr);
  ...
  var sql = "SELECT hex(idr) as idr, hash FROM Entries WHERE idr IN (" + entryInClause + ")";
  sql = unionize(partition, sql, []);
  if (exports.debug) logger.debug(sql);
  var start = Date.now();
  dal.query(sql, [], function(error, rows) {
...
exports.spoil = function(basePath, cbDone) {
  ...
  dal.query(sql, binds, function(){
    dal.query(sql.replace("Entries",tableize(idr.partition(basePath))), binds, cbDone); // do a second time for the partitions too
  });
};

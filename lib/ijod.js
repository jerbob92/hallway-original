...
exports.setOnePars = function(id, cat, pars, cbDone){
  ...
  var sql = "INSERT INTO Entries (base, idr, path, par) VALUES (unhex(concat(rpad(?,32,'0'), lpad(hex(?),12,'0'), substr(?,1,16))), unhex(?), ?, x?) ON DUPLICATE KEY UPDATE base=VALUES(base), path=VALUES(path), par=VALUES(par)";

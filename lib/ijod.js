...
exports.getPars = function(basePath, options, cbDone) {
  ...
  dal.query(sql, binds, function(err, rows) {
    ...
    rows.forEach(function(row) {
      ...
      var id;
      // extra parallel rows
      id = (row.at > now) ? row.hash : row.idr;
      if (!ret[id]) ret[id] = [];
    ...
    // when there was a search filter, we don't have all the pars yet, one more pass over just these ids
    options = {};
    options.in = Object.keys(ret);
    logger.debug("recursing to get all pars");
    exports.getPars(basePath, options, function(err, more){
      if(err || !more) return cbDone(null, ret);
      Object.keys(more).forEach(function(id){
        lutil.addAll(ret[id], more[id]);
      });
      cbDone(null, ret);
    });
  });
...
exports.setOnePars = function(id, cat, pars, cbDone){
  ...
  var binds = [idr.baseHash(id), at, idr.hash(id), friends.parCat(idr.hash(id), cat), idr.parse(id).hash, idr.hash(id), parclean.join('')];

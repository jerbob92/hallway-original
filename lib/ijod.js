...
exports.getPars = function(basePath, options, cbDone) {
  ...
  var q = options.q ? ", (true "+qq(options.q, true)+") as q" : ""; // do text search as a returned variable
  var sql = "SELECT conv(hex(substr(base,17,6)),16,10) as at, " +
    "lpad(hex(par),32,'0') as par, hex(idr) as idr, hash" + path + q + " FROM Entries WHERE " +

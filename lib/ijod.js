...
exports.getPars = function(basePath, options, cbDone) {
  ...
  dal.query(sql, binds, function(err, rows) {
    ...
    // first index all normal idrs
    rows.forEach(function(row) {
      if (!row.par) return;
      if(row.at <= now) ret[row.idr] = [];
    });
    ...
    rows.forEach(function(row) {
      ...
      // extra parallel rows
      if (row.at > now) {
        if(!ret[row.idr]) ret[row.idr] = []; // this only happens if there isn't a primary normal idr from above
...
exports.setOnePars = function(id, cat, pars, cbDone){
  ...
  var binds = [idr.baseHash(id), at, idr.hash(id), friends.parCat(idr.hash(id), cat), idr.parse(id).hash, pars.slice(0,4).join('')];

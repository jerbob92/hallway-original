...
IJOD.prototype.addData = function(arg, callback) {
  ...
  this.startAddTransaction(function() {
    ...
    var sql = "INSERT INTO Entries (base, idr, path, hash, offset, len, lat, lng, q0, q1, q2, q3, par) VALUES (unhex(concat(rpad(?,32,'0'), lpad(hex(?),12,'0'), substr(?,1,16))), unhex(?), ?, ?, ?, ?, ?, ?, "+qsql+") ON DUPLICATE KEY UPDATE base=VALUES(base), path=VALUES(path), hash=VALUES(hash), offset=VALUES(offset), len=VALUES(len), lat=VALUES(lat), lng=VALUES(lng), q0=VALUES(q0), q1=VALUES(q1), q2=VALUES(q2), q3=VALUES(q3), par=VALUES(par)";
    self.transactionQueries.push(dal.sqlize(sql, [idr.baseHash(arg.idr), arg.at, idr.hash(arg.idr), idr.hash(arg.idr), self.path, hash, offset, (self.len - offset), ll[0], ll[1], qx[0], qx[1], qx[2], qx[3], par]));
    ...
    async.forEachSeries(Object.keys(arg.types), function(type, cb){
      ...
      self.transactionQueries.push(dal.sqlize(sql, [idr.baseHash(i2), arg.at, idr.hash(i2), idr.hash(i2), self.path, hash, offset, (self.len - offset), ll[0], ll[1], qx[0], qx[1], qx[2], qx[3], par]));
      cb();
    }, callback);
  });
}
...
exports.delOne = function(targetIdr, callback)
{
  ...
  memcache.del(hash, function(error, result) {
    dal.query("DELETE FROM Entries WHERE idr = x? LIMIT 1", [hash], function(error) {
      callback();
    });
  });
}
  ...
  dal.query(sql, binds, function(error, rows) {
    ...
    function processPath(pathData, cbPath) {
      ...
      var req = s3client.get(pathData.path, {
        ...
        // XXX: I can't find a clean way to knowingly bail out of http data results in the middle, hence this flag -- temas
        var failed = false;
        res.on("data", function(chunk) {
          ...
          try {
            ...
            logger.error("Error copying a chunk (length %d) to full buffer position %d", chunk.length, fullBufWritePos);
            failed = true;
            return false;
          }
        ...
        res.on("end", function() {
          ...
          for (var i = 0; i < pathData.ranges.length; ++i) {
            ...
            if (!decompressed) {
              logger.error("S3 Error in %d for %s", res.statusCode, pathData.path);
...
console.warn("REMOVE ME, DEBUGGING, FIX NEXT LINE")
          if (false && hash == knownIds[entryIdrHash]) {
            return cb();
          } else {
            entry.hash = hash;
          }
...
exports.getPars = function(basePath, options, cbDone) {
  var sql = "SELECT conv(hex(substr(base,17,6)),16,10) as at, lpad(hex(par),32,'0') as par, hex(idr) as idr FROM Entries WHERE base > unhex(rpad(?,60,'0')) AND base < unhex(rpad(?,60,'f'))";
  ...
  dal.query(sql, binds, function(err, rows){
    ...
    var now = Date.now();
    function ndxr(to, id, part) { if(part != '00000000') to[id].push(part) }; // convenience
    rows.forEach(function(row){
      if(row.at > now) // extra parallel rows 
      {
        var id = row.idr.substr(0,31); // last nib is the variant, rest is prefix of real idr
        var to = ndx;
      }else{
        var id = row.idr;
        var to = ret;
    ...
    logger.debug("got ",Object.keys(ret).length,"idrs and",Object.keys(ndx).length,"extras");
...
function qget(entry)
{
  ...
  return [entry.q, (oe.type=='link')?oe.url:'', oe.title, oe.author_name, oe.email, dMap.get('text', entry.data, entry.idr)].join(" "); // get all queryable strings
}
exports.qtext = qget;

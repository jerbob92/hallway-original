...
exports.pump = function(arg, cbDone) {
  ...
  async.forEach(Object.keys(bases), function(base, cb) {
    ...
    var avg = (Date.now() - start) / arg.length;
    if ( avg > lconfig.ijod.slowPump ) {
      logger.warn('IJOD pump took > %d ms for each of %d entries, %j',
        lconfig.ijod.slowPump, arg.length, timings);
    }

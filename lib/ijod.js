...
// this is a stopgap, aren't all unions?
function unionize(base, sql, binds)
{
  var end = "";
  if(sql.toLowerCase().indexOf(" order by ") > 0)
  {
    var offset = sql.toLowerCase().indexOf(" order by ");
    end += sql.substr(offset);
    sql = sql.substr(0,offset);
  }
  if(sql.toLowerCase().indexOf(" limit ") > 0)
  {
    var offset = sql.toLowerCase().indexOf(" limit ");
    end += sql.substr(offset);
    sql = sql.substr(0,offset);
  }
  var copy = sql.replace("Entries",tableize(base));
  // put more specific partition reference first so newest applies to limit
  sql = ["(", copy, ") UNION (", sql, ")", end].join("");
  var blen = binds.length;
  // duplicate all binds for second copy
  for(var i = 0; i < blen; i++) binds.push(binds[i]);
  return sql;
}

// best to keep this in one place
function tableize(base)
{
  return 'Entries_'+base.substr(0,2).toLowerCase();
}
...
function idParse(id)
{
  ...
  if (id.indexOf(':') > 0)
  {
    table = tableize(idr.baseHash(id));
    hash = idr.hash(id);
  } else if (id.indexOf('_') > 0) {
    table = tableize(id.split('_')[1]);
    hash = id.split('_')[0];
  } else {
...
exports.getPars = function(basePath, options, cbDone) {
  ...
  sql = unionize(mmh.murmur128HexSync(basePath), sql, binds);
...
exports.getOnePars = function(targetIdr, cat, cbDone) {
  ...
  if (cat) { // just get one exact row
    entryInClause = "x'"+friends.parCat(idp.hash, cat)+"'";
  }else{
    entryInClause = Object.keys(friends.parCats()).map(function(cat) {
      return "x'" + friends.parCat(idp.hash, cat) + "'";
    }).join(",");
  }
...
IJOD.prototype.addData = function(arg, callback) {
  ...
  var sql = "INSERT INTO "+ tableize(idr.baseHash(arg.idr)) +" (base, idr, path, hash, offset, len, lat, " +
...
exports.batchSmartAdd = function(entries, callback) {
  ...
  var base = idr.baseHash(entries[0].idr);
...
exports.spoil = function(basePath, cbDone) {
  ...
  dal.query(sql, binds, function(){
    dal.query(sql.replace("Entries",tableize(mmh.murmur128HexSync(basePath))), binds, cbDone); // do a second time for the partitions too
  });
};

...
exports.getPars = function(basePath, options, cbDone) {
  ...
  dal.query(sql, binds, function(err, rows) {
    ...
    // fill the ndx w/ all possible matches
    rows.forEach(function(row) {
      if (!row.par) return;
      if(row.at <= now) ndx[row.idr.substr(0,31)] = [];
    });
    ...
    rows.forEach(function(row) {
      ...
      // extra parallel rows
      if (row.at > now) {
        id = row.idr.substr(0,31); // last nib is the variant, rest is prefix of real idr
        if(!ndx[id]) ret[row.idr] = []; // this only happens if there isn't a primary normal idr indexed from above
        to = ndx;
      }else{
        id = row.idr;
        to = ret;
    ...
    Object.keys(ret).forEach(function(idr) {
      lutil.addAll(ret[idr], ndx[idr.substr(0,31)]);
    });
    cbDone(null, ret);
  });
};
...
exports.setOnePars = function(id, cat, pars, cbDone){
  ...
  var sql = "INSERT INTO Entries (base, idr, path, par) VALUES (unhex(concat(rpad(?,32,'0'), lpad(hex(?),12,'0'), substr(?,1,16))), unhex(?), ?, unhex(?)) ON DUPLICATE KEY UPDATE base=VALUES(base), path=VALUES(path), par=VALUES(par)";
  var at = Date.now()+(10*365*24*3600*1000); // 10yrs into the future, as a way to make sure they don't show up as a normal entry
  var binds = [idr.baseHash(id), at, idr.hash(id), friends.parCat(idr.hash(id), cat), idr.parse(id).hash, parclean.join('')];

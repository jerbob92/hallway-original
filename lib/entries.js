      ...
      if(options.dedup) {
        ...
        batch.forEach(function(item) {
          ...
          item.guid.split(' ').forEach(function(guid){
            (guids[guid]) ? dup = guid : guids[guid] = item.id; // save oldest id for debugging
          });
...
exports.bases = function(path, query, profilesIN)
{
  ...
  if(parts[1] == 'services')
  {
    ...
    profiles.forEach(function(pid){
      ...
      if(!endpoint) return dMap.bases([pid]).forEach(function(base){bases.push(base)}); // support /services/:service to get all
...
exports.toString = function(entry, options)
{
  ...
  dMap.typeOf(entry.idr, entry.types).forEach(function(type){ entry.types[type] = true }); // get all the possible types
  ...
  Object.keys(entry.types).forEach(function(type){ if(PLURAL[type]) entry.types[PLURAL[type]] = true }); // to be nice and provide singular names consistently

...
exports.typeOf = function(id, types)
{
  ...
  if(maps[id.host])
  {
    var tmap = maps[id.host]['types'];
    if(tmap) Object.keys(tmap).forEach(function(type){
      if(tmap[type].indexOf(key) >= 0) ret[type] = true;
    });
  }else if(maps.system.defaults[id.path] == id.protocol){ // might be a system loaded type
    ret[maps.system.types[id.path]] = true;
  }
  // if there's other internal types, map those too
  if(types) Object.keys(types).forEach(function(type){
    id.protocol = type;
    exports.typeOf(id).forEach(function(t){ ret[t] = true });
  });
...
exports.defaultcheck = function(service, context, type)
{
  ...
  ijod.getOne(mapidr, function(err, entry) {
    ...
    var entry = {data:map, at:Date.now(), idr:mapidr};
...
// load a custom one that is a fallback TODO replace the maps.system builtins with this
exports.loadcustom = function(type, path, map) {
  maps.system.defaults[path] = type;
  maps.system[type] = map;
  if(map.type) maps.system.types[path] = map.type; // allow for the stupid plural friendly versions like /types/contacts variant
}

...
exports.loadcheck = function(service, callback) {
  if(map[service] && map[service]._system) return callback(); // pass-through system loaded ones
  var idr = 'map:'+service+'/maps#default';
  ijod.getOne(idr, function(err, entry){
    if(!entry || !entry.data || typeof entry.data.contents != 'string') return callback();
    if(map[service] && map[service]._loaded == entry.at) return callback(); // hasn't changed
    // time to load a new one, there's probably a better way but I'm offline atm
    var tmp = '/tmp/hallway-'+service+'-map.js';
    fs.writeFileSync(tmp,entry.data.contents);
    try {
      maps[service] = require(tmp);

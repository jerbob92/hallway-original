...
exports.pump = function (cset, callback) {
  async.forEachSeries(cset, function (entry, cbStep) {
    var r = idr.parse(entry.idr);
    var svc = maps[r.host] || maps.system;
    if (!svc.pumps) return process.nextTick(cbStep);
...
exports.loadcheck = function (service, callback) {
  ...
  podClient.getOne(idr, function (err, entry) {
...
exports.defaultcheck = function (service, context, type) {
  ...
  podClient.getOne(mapidr, function (err, entry) {
...
exports.load = function () {
  ...
  Object.keys(partypes).forEach(function (type) {
    if (partyped[partypes[type]]) {
      // XXX: This seems like it could cause corruption... Should we exit?
      logger.warn("conflicting type!", type, partypes[type],
        partyped[partypes[type]]);
    }

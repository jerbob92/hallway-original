...
// our own optional in-memory cache of apps since they are used often and change rarely
var APPCACHE = {};
// refresh the cache hourly
setInterval(function(){
  async.forEachSeries(Object.keys(APPCACHE), function(appId, cbApps){
    exports.getApp(appId, false, function(){ cbApps(); });
  }, function(){});
}, 3600000);
...
exports.getApp = function(app, useCache, callback) {
  ...
  logger.debug("getting app ",app,useCache,typeof APPCACHE[app]);
...
function appCustomFreq(app, service, synclet, useCache, callback) {
  exports.getApp(app, useCache, function(err, appInfo) {
...
exports.customFreq = function(apps, service, synclet, useCache, callback) {
  if (!Array.isArray(apps)) {
    return process.nextTick(callback.bind(null, 'apps is not an array!'));
  }
  var cfs = [];
  async.forEach(apps, function(app, cbLoop) {
    appCustomFreq(app, service, synclet, useCache, function(err, customFreq) {

...
exports.getApp = function(app, callback, refresh) {
  if (role === 'nexus' || !lconfig.pods.enabled || refresh) {
    return getAppLocal(app, callback, refresh);
  }

  return nexusClient.getApp(app, callback);
};

function getAppLocal(app, callback, refresh) {
...
exports.getAppClasses = function(app, callback) {
  ...
  function doApp(err, app) {
    ...
    if (lconfig.synclist && !lconfig.synclist[app.clientID]) {
      return process.nextTick(callback.bind(null, null, {}));      
    }
    // don't sync any blacklisted apps here
    if (lconfig.syncskip && lconfig.syncskip[app.clientID]) {
      return process.nextTick(callback.bind(null, null, {}));      
    }
    // make a class for each synclet in the custom frequency alone
    if (app.notes.customOnly && typeof app.notes.customFreqs !== 'object') logger.warn("misconfigured app, customOnly and no customFreqs", app.clientID);

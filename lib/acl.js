...
exports.customFreq = function(apps, service, synclet, callback) {
  ...
  async.forEachSeries(apps, function(app, cbLoop) {
    ...
    if (lconfig.synclist && !lconfig.synclist[app.app]) return cbLoop();
    // don't sync any blacklisted apps here
    if (lconfig.syncskip && lconfig.syncskip[app.app]) return cbLoop();
    ...
    if (err) return callback(err);
    if(cfs.length == 0) cfs.push(defaultFreq(service, synclet)); // default if none else
    var freq = arrMin(cfs);
    callback(null, freq);
  });
};

...
exports.getAppsForAccount = function(account, callback) {
  ...
  dal.query("SELECT Apps.app, Apps.secret, Apps.apikeys, Apps.notes FROM Apps, Owners WHERE Apps.app = Owners.app and Owners.account = ?", [account], function(err, rows) {
    var apps = [];
...
// just fetch the info for a given app id, refresh keeps logic to update cache in one place
  ...
  redis.get(app, function(error, result){
    ...
    dal.query(q, [app], function(err, rows) {
      ...
      if (rows[0]) {
        ...
        redis.set(app, JSON.stringify(rows[0]), function(){}); // save in the background
      }
      callback(err, rows[0]);
    });
  });
